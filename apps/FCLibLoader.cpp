...
static double solve(const fclib_local* problem, const Eigen::SparseMatrixBase<EigenDerived>& ei_W, Options& options,
                    Eigen::VectorXd& r, Eigen::VectorXd& u, Stats& stats) {
...
int main(int argc, const char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (argv[i][0] == '-') {
      switch (argv[i][1]) {
        ...
        case 'a':
          if (++i == argc) break;
          options.algorithm = (bogus::fclib::Options::Algorithm)std::atoi(argv[i]);
          break;
        case 'g':
          if (++i == argc) break;
          options.pgVariant = (bogus::projected_gradient::Variant)std::atoi(argv[i]);
          break;
        case 'r':
          if (++i == argc) break;
  ...
  if (options.maxThreads > 1) options.gsColoring = true;
  ...
  if (problem) {
    ...
    if (problem->W && !problem->V && !problem->R) {
      std::cout << " Pure " << d << "D Coulomb friction problem with " << n << " contacts " << std::endl;

      if (problem->W->nz == -2) {
        std::cout << " Compressed row storage " << problem->W->m << " / " << problem->W->n << " / " << problem->W->nzmax
                  << std::endl;
