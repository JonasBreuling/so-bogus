...
struct Options {
  enum Algorithm { GaussSeidel, ProjectedGradient };
...
struct Stats {
  ...
  explicit Stats(bool v = false) : verbose(v), nIters(0), error(-1), time(0) {}
  ...
  void update(unsigned it, double err) {
    ...
    error = err;
    time = timer.elapsed();
    if (verbose) std::cout << it << ":\t" << err << "\t" << time << std::endl;
    callback.trigger(it, err, time);
  }
};

template <unsigned Dimension>
double solveDual(const DualFrictionProblem<Dimension> &dual, const Options &options, Eigen::VectorXd &r,
                 Eigen::VectorXd &u, Stats &stats) {
