...
int main() {
...

  {
    ...
    bogus::Krylov<bogus::SparseBlockMatrix<Block> > solver(A);

    solver.setMaxIters(1000);
    solver.setTol(1e-12);

    solver.solve(b, x, bogus::krylov::CG);
...

  {
    ...
    bogus::ProjectedGradient<bogus::SparseBlockMatrix<Block> > solver(A);

    solver.setMaxIters(1000);
    solver.setTol(1e-12);

    solver.solve(bogus::NoneLaw<double>(), b, x);
...

  {
    ...
    bogus::GaussSeidel<bogus::SparseBlockMatrix<Block> > solver(A);
