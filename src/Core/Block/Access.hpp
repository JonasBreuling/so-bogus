...
inline typename std::enable_if_t<BlockTraits<SelfTransposeT>::is_self_transpose, const SelfTransposeT&> transpose_block(
    const SelfTransposeT& block) {
  return block;
}
...
inline
    typename std::enable_if_t<!BlockTraits<BlockType>::is_self_transpose, typename BlockType::ConstTransposeReturnType>
...
/*! Process af follow:
  ...
  */
template <typename BlockType, bool IsSelfTranspose = BlockTraits<BlockType>::is_self_transpose,
...
struct Segmenter {
  ...
  inline ReturnType operator[](const Index inner) { return m_vec.template middleRows<dimension>(dimension * inner); }
