...
class BlockMatrixBase : public IterableBlockObject<Derived> {
  ...
  */
  ...
  BlockPtr blockPtr(Index row, Index col) const { return derived().blockPtr(row, col); }
  //! Return a BlockPtr to the block a (row, row) or InvalidBlockPtr if it does not exist
  BlockPtr diagonalBlockPtr(Index row) const { return derived().diagonalBlockPtr(row); }
  ...
  //! \warning block has to exist
  ConstBlockRef diagonal(const Index row) const { return block(diagonalBlockPtr(row)); }

  //! \warning block has to exist
  BlockRef block(Index row, Index col) { return block(blockPtr(row, col)); }
  //! \warning block has to exist
  ConstBlockRef block(Index row, Index col) const { return block(blockPtr(row, col)); }
