...
struct CompoundSparseBlockIndex
  ...
  CompoundSparseBlockIndex(const SparseBlockIndexBase<FirstIndexType>& index1,
      ...
      : Base(index1.valid && index2.valid),
        first(index1.derived()),
        second(index2.derived()),
  ...
  Index size(Index outerIdx) const { return first.size(outerIdx) + second.size(outerIdx); }
...
struct SparseBlockIndexTraits<CompoundSparseBlockIndex<FirstIndexType, SecondIndexType, NativeOrder> > {
  ...
  struct InnerIterator {
    ...
    operator bool() const { return m_it1 || m_it2; }
    ...
    Index inner() const { return m_it1 ? m_it1.inner() : m_it2.inner(); }
    BlockPtr ptr() const { return m_it1 ? m_it1.ptr() : m_it2.ptr(); }
    ...
    bool after(Index outer) const { return NativeOrder ? (inner() > outer) : (inner() < outer); }
