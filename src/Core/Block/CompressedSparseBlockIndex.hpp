...
struct SparseBlockIndex<true, Index_, BlockPtr_, ArrayType>
  ...
  void insert(Index outIdx, Index inIdx, BlockPtr ptr) {
    static_assert(Ordered, "UNORDERED_INSERTION_WITH_COMPRESSED_INDEX");

    valid &= (ptr == (BlockPtr)(inner.size())) && (0 == outer[outIdx + 1] || inIdx > inner.back());
    ++outer[outIdx + 1];
    inner.push_back(inIdx);
  }
  void insertBack(Index outIdx, Index inIdx, BlockPtr ptr) { insert<true>(outIdx, inIdx, ptr); }
  ...
  */
  ...
  SparseBlockIndex& operator=(const SparseBlockIndex& compressed) {
    if (&compressed != this) {
      ...
      if (!compressed.innerOffsets.empty()) innerOffsets = compressed.innerOffsets;
  ...
  SparseBlockIndex& operator=(const SparseBlockIndexBase<SourceDerived>& source) {
    ...
    for (typename SourceDerived::Index i = 0; i < source.outerSize(); ++i) {
      for (typename SourceDerived::InnerIterator it(source.derived(), i); it; ++it) {
  ...
  SparseBlockIndex& move(SparseBlockIndex& compressed) {
    if (&compressed != this) {
      ...
      if (!compressed.innerOffsets.empty()) innerOffsets.swap(compressed.innerOffsets);
      valid = compressed.valid;
  ...
  Index size(const Index outerIdx) const { return outer[outerIdx + 1] - outer[outerIdx]; }

  // MKL BSR
  const Index* rowIndex() const { return outer.data(); }
  const Index* columns() const { return inner.data(); }

  // Same with nicer names
  const Index* outerIndexPtr() const { return outer.data(); }
  const Index* innerIndexPtr() const { return inner.data(); }
};
...
struct SparseBlockIndexTraits<SparseBlockIndex<true, Index_, BlockPtr_, ArrayType> > {
  ...
  struct InnerIterator {
    ...
    InnerIterator(const SparseBlockIndexType& index, Index outer)
        : m_it(index.outer[outer]), m_end(index.outer[outer + 1]), m_inner(index.inner.data()) {}
