...
struct bindings< float >
{
        ...
        static void bsrmv (char *transa, MKL_INT *m, MKL_INT *k, MKL_INT *lb, Scalar *alpha, char *matdescra, Scalar
*val, MKL_INT *indx,  MKL_INT *pntrb, MKL_INT *pntre, Scalar *x, Scalar *beta, Scalar *y)
        {
...
template <typename Scalar>
void bsrmv(bool Symmetric, bool Transpose, MKL_INT Dimension, MKL_INT m, MKL_INT k, const std::size_t offset,
           const MKL_INT *rowIndex, const MKL_INT *columns, const Scalar *data, const Scalar *rhs, int rhsCols,
           Scalar *res, Scalar alpha, Scalar beta) {
  ...
  for (int i = 0; i < rhsCols; ++i) {
    bindings<Scalar>::bsrmv(&transa, &m, &k, &lb, &alpha, matdescra, a, indx, pntrb, pntre, x + i * Dimension * k,
                            &beta, y + i * Dimension * k);
  }
}

template <typename BlockPtr, typename BlockType>
static void rowmv(const SparseBlockIndex<true, MKL_INT, BlockPtr> &index, const BlockType *data, MKL_INT row,
  ...
  mkl::bsrmv<typename BlockTraits<BlockType>::Scalar>(false, false, BlockTraits<BlockType>::RowsAtCompileTime, 1,
...
struct SparseBlockMatrixOpProxy<true, true, double, MKL_INT> {
  ...
  static void multiply(const SparseBlockMatrixBase<Derived> &matrix, const RhsT &rhs, ResT &res, Scalar alpha,
    ...
    mkl::bsrmv<Scalar>(Traits::is_symmetric, Transpose, Derived::RowsPerBlock, matrix.rowsOfBlocks(),
                       ...
                       data_pointer(matrix.data()[0]), rhs.data(), rhs.cols(), res.data(), alpha, beta);
  }
  ...
  static void splitRowMultiply(const SparseBlockMatrixBase<Derived> &matrix, typename Derived::Index row,
    ...
    mkl::rowmv(matrix.majorIndex(), matrix.data(), row, rhs.data(), rhs.cols(), res.data());

    if (Traits::is_symmetric) {
      mkl::rowmv(matrix.transposeIndex(), matrix.data(), row, rhs.data(), rhs.cols(), res.data());
    }
