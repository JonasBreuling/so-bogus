  ...
  if (m_majorIndex.valid) {
    if (useTransposeIndex) {
      ...
      if (m_transposeIndex.valid) {
        ...
        BlockCopier<needTranspose>::copy(source.transposeBlocks(), m_transposeBlocks, source.transposeBlocks().size(),
                                         scale);
      }
    }
    ...
    for (Index i = 0; i < sourceIndex.outerSize(); ++i) {
      for (typename SourceIndexType::InnerIterator src_it(sourceIndex, i);
        ...
        TransposeIf::assign(source.block(src_it.ptr()), dest, scale, src_it.after(i));
      }
    }
    finalize();
  }
