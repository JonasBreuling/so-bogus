...
#pragma omp parallel for
  ...
  SparseBlockIndex& move(SparseBlockIndex& uncompressed) {
    if (&uncompressed != this) {
      ...
      if (!uncompressed.innerOffsets.empty()) innerOffsets.swap(uncompressed.innerOffsets);
  ...
  SparseBlockIndex& setToTranspose(const SparseBlockIndexBase<SourceDerived>& source) {
    ...
    for (typename SourceDerived::Index i = 0; i < source.outerSize(); ++i) {
      ...
      for (typename SourceDerived::InnerIterator it(source.derived(), i); it && (!Symmetric || it.inner() < i); ++it) {
  ...
  void changePtr(const InnerIterator& it, BlockPtr ptr) { const_cast<BlockPtr&>(it.asStdIterator()->second) = ptr; }
};
...
struct SparseBlockIndexTraits<SparseBlockIndex<Compressed, Index_, BlockPtr_, ArrayType> > {
  ...
  struct InnerIterator {
    ...
    difference_type operator-(const InnerIterator& other) const { return m_it - other.m_it; }
