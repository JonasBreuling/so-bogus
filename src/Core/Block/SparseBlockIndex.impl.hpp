...
template <typename Derived>
const typename SparseBlockIndexBase<Derived>::InnerOffsetsType& SparseBlockIndexBase<Derived>::innerOffsetsArray()
    const {
...
SparseBlockIndex<Compressed, Index, BlockPtr, ArrayType>::operator=(const SparseBlockIndexBase<SourceDerived>& source) {
  clear();
