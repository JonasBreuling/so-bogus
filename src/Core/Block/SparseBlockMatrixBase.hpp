...
class SparseBlockMatrixBase : public BlockMatrixBase<Derived> {
  ...
  */
  ...
  void setRows(const std::vector<unsigned>& rowsPerBlock) { setRows(rowsPerBlock.size(), &rowsPerBlock[0]); }
  //! Same, setting each block to have exactly \p rowsPerBlock
  void setRows(const Index nBlocks, const Index rowsPerBlock) { setRows(std::vector<unsigned>(nBlocks, rowsPerBlock)); }
  ...
  */
  ...
  //! Same, using a std::vector
  void setCols(const std::vector<unsigned>& colsPerBlock) { setCols(colsPerBlock.size(), &colsPerBlock[0]); }
  //! Same, setting each block to have exactly \p rowsPerBlock
  void setCols(const Index nBlocks, const Index colsPerBlock) { setCols(std::vector<unsigned>(nBlocks, colsPerBlock)); }
  //! Same, deducing the (constant) number of rows per block from the BlockType
  ...
  //! Convenience method that insertBack() a block and immediately resize it according to the dimensions given to
  //! setRows() and setCols()
  ...
  */
  BlockRef insert(Index row, Index col) {
    static_assert(!Traits::is_compressed, "UNORDERED_INSERTION_WITH_COMPRESSED_INDEX");
    if (Traits::is_col_major)
  ...
  //! Convenience method that insert() a block and immediately resize it according to the dimensions given to setRows()
  //! and setCols()
  ...
  */
  ...
  /*!
    ...
    \sa \ref block_access
   */
  InnerIterator innerIterator(Index outer) const { return InnerIterator(m_majorIndex, outer); }
  ...
  */
  ...
  void reserve(std::size_t nBlocks, std::size_t totalNonZeros) { derived().reserve(nBlocks, totalNonZeros); }
  ...
  const TransposeArrayType& transposeBlocks() const { return m_transposeBlocks; }

  //! Returns the blocks that have been created by cacheTranspose(), as a raw pointer
  const TransposeBlockType* transposeData() const { return data_pointer(m_transposeBlocks); }
 ...
 protected:
  enum {
    is_bsr_compatible = Base::has_fixed_size_blocks && Base::has_square_or_dynamic_blocks && Traits::is_compressed &&
                        !Traits::is_col_major && BlockTraits<BlockType>::uses_plain_array_storage
  };
  ...
  TransposeBlockType* transposeData() { return data_pointer(m_transposeBlocks); }
  ...
  void createBlockShapes(const BlockPtr nBlocks, const IndexType& index, typename Traits::BlocksArrayType& blocks) {
