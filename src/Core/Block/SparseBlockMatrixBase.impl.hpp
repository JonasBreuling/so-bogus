...
template <typename Derived>
void SparseBlockMatrixBase<Derived>::setRows(const Index nBlocks, const unsigned* rowsPerBlock) {
...
template <typename Derived>
void SparseBlockMatrixBase<Derived>::setCols(const Index nBlocks, const unsigned* colsPerBlock) {
...
void SparseBlockMatrixBase<Derived>::setInnerOffets(IndexT& index, const Index nBlocks,
...
typename SparseBlockMatrixBase<Derived>::BlockRef SparseBlockMatrixBase<Derived>::insertByOuterInner(Index outer,
                                                                                                     Index inner) {
  ...
  BlockRef b = derived().template allocateBlock<!Ordered>(
      ptr, m_minorIndex.innerOffsetsData()[outer + 1] - m_minorIndex.innerOffsetsData()[outer],
...
typename SparseBlockMatrixBase<Derived>::BlockRef SparseBlockMatrixBase<Derived>::allocateBlock(BlockPtr& ptr,
                                                                                                Index outer,
                                                                                                Index inner) {
...
template <typename Derived>
const typename SparseBlockMatrixBase<Derived>::MinorIndexType& SparseBlockMatrixBase<Derived>::getOrComputeMinorIndex(
...
void SparseBlockMatrixBase<Derived>::cacheTranspose() {
  static_assert(IsTransposable<typename Derived::BlockType>::Value, "TRANSPOSE_IS_NOT_DEFINED_FOR_THIS_BLOCK_TYPE");
...
#pragma omp parallel for
  ...
  for (Index i = 0; i < m_minorIndex.outerSize(); ++i) {
    ...
    for (typename TransposeIndexType::InnerIterator it(m_transposeIndex, i); it; ++it, ++uncompressed_it) {
...
template <typename Derived>
typename SparseBlockMatrixBase<Derived>::BlockPtr SparseBlockMatrixBase<Derived>::diagonalBlockPtr(
...
typename SparseBlockMatrixBase<Derived>::BlockPtr SparseBlockMatrixBase<Derived>::blockPtr(Index row, Index col) const {
  ...
  const typename MajorIndexType::InnerIterator found(std::lower_bound(innerIt, innerIt.end(), col));
...
void SparseBlockMatrixBase<Derived>::cloneStructure(const SparseBlockMatrixBase<OtherDerived>& source) {
  static_assert(static_cast<unsigned>(BlockMatrixTraits<Derived>::flags) ==
...
Derived& SparseBlockMatrixBase<Derived>::applyPermutation(const std::size_t* indices) {
  ...
  for (Index outer = 0; outer < sourceIndex.outerSize(); ++outer) {
    for (typename MajorIndexType::InnerIterator inner(sourceIndex, indices[outer]); inner; ++inner) {
  ...
  for (Index outer = 0; outer < destIndex.outerSize(); ++outer) {
    for (typename UncompressedIndexType::InnerIterator inner(destIndex, outer); inner; ++inner) {
...
template <typename Derived>
typename SparseBlockMatrixBase<Derived>::BlockRef SparseBlockMatrixBase<Derived>::insertBackAndResize(Index row,
                                                                                                      Index col) {
...
template <typename Derived>
typename SparseBlockMatrixBase<Derived>::BlockRef SparseBlockMatrixBase<Derived>::insertAndResize(Index row,
                                                                                                  Index col) {
