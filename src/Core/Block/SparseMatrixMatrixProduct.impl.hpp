...
struct SparseBlockProductTerm {
  ...
  SparseBlockProductTerm(Index idx, BlockPtr lPtr, BlockPtr rPtr, bool lIAD, bool rIAD)
      : index(idx), lhsPtr(lPtr), rhsPtr(rPtr), lhsIsAfterDiag(lIAD), rhsIsAfterDiag(rIAD) {}

  // Ordering so all terms for a single final block are consecutive
  bool operator<(const SparseBlockProductTerm &rhs) const { return index < rhs.index; }
};
...
struct SparseBlockProductIndex {
  ...
  void compute(const Index outerSize, const Index innerSize, const LhsIndex &lhsIdx, const RhsIndex &rhsIdx) {
        ...
        while (lhs_it && rhs_it) {
          if (lhs_it.inner() > rhs_it.inner())
            ...
            to_compute[i].push_back(
                Term(j, lhs_it.ptr(), rhs_it.ptr(), lhs_it.after(lhsOuter), rhs_it.after(rhsOuter)));
            nonZero = true;
            ++lhs_it;
            ++rhs_it;
          }
      ...
      if (is_col_major) {
        for (typename RhsIndex::InnerIterator rhs_it(rhsIdx, i); rhs_it; ++rhs_it) {
          for (typename LhsIndex::InnerIterator lhs_it(lhsIdx, i);
               lhs_it && (!is_symmetric || lhs_it.inner() <= rhs_it.inner()); ++lhs_it) {
            loc_compute[rhs_it.inner()].push_back(
                Term((Index)lhs_it.inner(), lhs_it.ptr(), rhs_it.ptr(), lhs_it.after(i), rhs_it.after(i)));
          }
        }
      } else {
        for (typename LhsIndex::InnerIterator lhs_it(lhsIdx, i); lhs_it; ++lhs_it) {
          for (typename RhsIndex::InnerIterator rhs_it(rhsIdx, i);
               rhs_it && (!is_symmetric || rhs_it.inner() <= lhs_it.inner()); ++rhs_it) {
            loc_compute[lhs_it.inner()].push_back(
                Term((Index)rhs_it.inner(), lhs_it.ptr(), rhs_it.ptr(), lhs_it.after(i), rhs_it.after(i)));
          }
        }
      }
    }
...
template <bool LhsRuntimeTest, bool RhsRunTimeTest, bool LhsCompileTimeTranspose, bool RhsCompileTimeTranspose>
struct BinaryTransposeOption {
  template <typename LhsT, typename RhsT, typename ResT>
  static inline void mm_assign(const LhsT &lhs, const RhsT &rhs, ResT &res, bool transposeLhs, bool transposeRhs) {
    if (transposeLhs)
      if (transposeRhs)
  ...
  template <typename LhsT, typename RhsT, typename ResT>
  static inline void mm_add(const LhsT &lhs, const RhsT &rhs, ResT &res, bool transposeLhs, bool transposeRhs) {
    if (transposeLhs)
      if (transposeRhs)
        res += transpose_block(lhs) * transpose_block(rhs);
      else
...
struct BinaryTransposeOption<false, true, LhsTranspose, RhsTranspose> {
  template <typename LhsT, typename RhsT, typename ResT>
  static inline void mm_assign(const LhsT &lhs, const RhsT &rhs, ResT &res, bool, bool transposeRhs) {
    if (transposeRhs)
  ...
  template <typename LhsT, typename RhsT, typename ResT>
  static inline void mm_add(const LhsT &lhs, const RhsT &rhs, ResT &res, bool, bool transposeRhs) {
    if (transposeRhs)
      res += TransposeIf<LhsTranspose>::get(lhs) * transpose_block(rhs);
    else
      res += TransposeIf<LhsTranspose>::get(lhs) * rhs;
...
struct BinaryTransposeOption<true, false, LhsTranspose, RhsTranspose> {
  template <typename LhsT, typename RhsT, typename ResT>
  static inline void mm_assign(const LhsT &lhs, const RhsT &rhs, ResT &res, bool transposeLhs, bool) {
    if (transposeLhs)
  ...
  template <typename LhsT, typename RhsT, typename ResT>
  static inline void mm_add(const LhsT &lhs, const RhsT &rhs, ResT &res, bool transposeLhs, bool) {
    if (transposeLhs)
      res += transpose_block(lhs) * TransposeIf<RhsTranspose>::get(rhs);
    else
      res += lhs * TransposeIf<RhsTranspose>::get(rhs);
...
struct BinaryTransposeOption<false, false, LhsTranspose, RhsTranspose> {
  template <typename LhsT, typename RhsT, typename ResT>
  static inline void mm_assign(const LhsT &lhs, const RhsT &rhs, ResT &res, bool, bool) {
    res = TransposeIf<LhsTranspose>::get(lhs) * TransposeIf<RhsTranspose>::get(rhs);
  }

  template <typename LhsT, typename RhsT, typename ResT>
  static inline void mm_add(const LhsT &lhs, const RhsT &rhs, ResT &res, bool, bool) {
    res += TransposeIf<LhsTranspose>::get(lhs) * TransposeIf<RhsTranspose>::get(rhs);
  }
};

template <typename TransposeOption, typename ResBlockRef, typename ProductIndex, typename LhsBlocks, typename RhsBlocks,
...
#pragma omp parallel for
  ...
  for (std::ptrdiff_t i = 0; i < (std::ptrdiff_t)nBlocks; ++i) {
    ...
    TransposeOption::mm_assign(lhsBlocks[bc->lhsPtr], rhsBlocks[bc->rhsPtr], b, bc->lhsIsAfterDiag, bc->rhsIsAfterDiag);

    for (unsigned j = 1; j != n; ++j) {
      ++bc;
      TransposeOption::mm_add(lhsBlocks[bc->lhsPtr], rhsBlocks[bc->rhsPtr], b, bc->lhsIsAfterDiag, bc->rhsIsAfterDiag);
    }
...
void SparseBlockMatrixBase<Derived>::setFromProduct(const Product<LhsT, RhsT> &prod) {
  ...
  static_assert(!Prod::transposeRhs || IsTransposable<typename RhsTraits::BlockType>::Value,
                "TRANSPOSE_IS_NOT_DEFINED_FOR_THIS_BLOCK_TYPE");
...

  {
    ...
    {
      SparseBlockIndexComputer<typename Prod::PlainLhsMatrixType, ColWise, Prod::transposeLhs> lhsIndexComputer(*lhs);
      SparseBlockIndexComputer<typename Prod::PlainRhsMatrixType, !ColWise, Prod::transposeRhs> rhsIndexComputer(*rhs);

      productIndex.compute(majorIndex().outerSize(), minorIndex().outerSize(), lhsIndexComputer.get(),
                           rhsIndexComputer.get());
    }
