...
//! Dual AMA iterative solver (Alternating Minimization Algorithm  on dual formuation of quadratic optimization
//! problem).
/*!
...
class DualAMA : public ConstrainedSolverBase<DualAMA<BlockMatrixType>, BlockMatrixType> {
 public:
  ...
  Scalar solve(const NSLaw &law, const BlockObjectBase<MatrixT> &A, const RhsT &f, const RhsT &w, ResT &v,
               ResT &r) const;
  ...
  template <admm::Variant variant, typename NSLaw, typename AType, typename BType, typename HType, typename PrecondT,
            typename RhsT, typename ORhsT, typename ResT, typename OResT>
  Scalar solveWithLinearConstraints(const NSLaw &law, const BlockObjectBase<AType> &A, const BlockObjectBase<BType> &B,
                                    const BlockObjectBase<HType> &H, const PrecondT &preconditioner, const RhsT &f,
                                    const ORhsT &k, const RhsT &w, ResT &v, OResT &p, ResT &r,
                                    Scalar stepRatio = 1) const;
  ...
  void setLineSearchIterations(const unsigned lsIterations) { m_lsIters = lsIterations; }

  //! Sets the amount by which the step size will be multiplied at the beginninf of each PG iteration.
  //! Should be greater than 1
  void setLineSearchOptimisticFactor(const Scalar lsOptimisticFactor) { m_lsOptimisticFactor = lsOptimisticFactor; }

  //! Sets the amount by which the step size will be multiplied at the end of each line-search iterations.
  //! Should be in ]0,1[
  void setLineSearchPessimisticFactor(const Scalar lsPessimisticFactor) { m_lsPessimisticFactor = lsPessimisticFactor; }
