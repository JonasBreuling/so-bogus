...
typename ADMM<BlockMatrixType>::Scalar ADMM<BlockMatrixType>::solve(const NSLaw &law, const ProxOp &op, const RhsT &w,
                                                                    ResT &v, ResT &r) const {
...
typename DualAMA<BlockMatrixType>::Scalar DualAMA<BlockMatrixType>::solve(const NSLaw &law,
                                                                          const BlockObjectBase<MatrixT> &A,
                                                                          const RhsT &f, const RhsT &w, ResT &v,
                                                                          ResT &r) const {
  switch (m_defaultVariant) {
    case admm::Accelerated:
...
typename DualAMA<BlockMatrixType>::Scalar DualAMA<BlockMatrixType>::solveWithLinearConstraints(
  ...
  typename GlobalProblemTraits::DynVector z(v.rows()), x, ut, gap, HrBp(v.rows()), g, g2, s(r.rows());

  const Segmenter<NSLaw::dimension, const DynVec, typename BlockMatrixType::Index> utSegmenter(ut,
                                                                                               m_matrix->rowOffsets());
  Segmenter<NSLaw::dimension, DynVec, typename BlockMatrixType::Index> sSegmenter(s, m_matrix->rowOffsets());
