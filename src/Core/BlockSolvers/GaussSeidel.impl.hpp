...
GaussSeidel<BlockMatrixType> &GaussSeidel<BlockMatrixType>::setMatrix(const BlockObjectBase<BlockMatrixType> &M) {
  if (m_matrix != &M && (m_matrix != nullptr || m_coloring.size() != (std::size_t)M.rowsOfBlocks())) {
...
#pragma omp parallel for
  ...
  for (Index i = 0; i < n; ++i) {
    ...
    if (ptr == BlockMatrixType::InvalidBlockPtr) {
      ...
      m_localMatrices[i] =
          MatrixTraits<typename BlockMatrixType::BlockType>::asConstMatrix(Base::explicitMatrix().block(ptr));
    }
  }
...
void GaussSeidel<BlockMatrixType>::innerLoop(bool parallelize, const NSLaw &law, const RhsT &b,
  ...
  const Segmenter<dimension, const RhsT, typename BlockMatrixType::Index> bSegmenter(b, m_matrix->rowOffsets());
...
#pragma omp for
    ...
    for (std::ptrdiff_t pi = m_coloring.colors[c]; pi < m_coloring.colors[c + 1]; ++pi) {
      ...
      if (std::min(nx2, ndx2) < absSkipTol || ndx2 < m_skipTol * std::min(nx2, ndxRef)) {
        skip[i] = absSkipIters;
      }
    }
  }
...
template <typename BlockMatrixType>
template <typename NSLaw, typename RhsT, typename ResT>
typename GaussSeidel<BlockMatrixType>::Scalar GaussSeidel<BlockMatrixType>::solve(const NSLaw &law, const RhsT &b,
                                                                                  ResT &x, bool tryZeroAsWell) const {
...
typename GaussSeidel<BlockMatrixType>::Scalar GaussSeidel<BlockMatrixType>::solveWithLinearConstraints(
    const NSLaw &law, const BlockObjectBase<LSDerived> &Cinv, const BlockObjectBase<HDerived> &H, const Scalar alpha,
    const RhsT &b, const RhsT &c, ResT &x, bool tryZeroAsWell, unsigned solveEvery) const {
...
typename GaussSeidel<BlockMatrixType>::Scalar GaussSeidel<BlockMatrixType>::solveWithLinearConstraints(
    const NSLaw &law, const BlockObjectBase<WDerived> &W, const RhsT &b, ResT &x, bool tryZeroAsWell,
