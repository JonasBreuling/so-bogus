...
class GaussSeidelBase : public ConstrainedSolverBase<GaussSeidelImpl, BlockMatrixType> {
  ...
  */
  ...
  /*!
          ...
          For under-determined problems, regularization might helps preventing \b x reaching problematically high
     values. Setting \f$\alpha\f$ to a too big value will however degrade the convergence of the global algorithm.
 ...
 protected:
  GaussSeidelBase()
      : m_maxThreads(0), m_evalEvery(25), m_skipIters(10), m_autoRegularization(0.), m_tryZeroAsWell(true) {
    m_tol = 1.e-6;
    m_maxIters = 250;
    m_skipTol = 1.e-6;
  }
  ...
  Scalar evalAndKeepBest(const NSLaw &law, const ResT &x, const typename GlobalProblemTraits::DynVector &y,
                         typename GlobalProblemTraits::DynVector &x_best, Scalar &err_best) const;

  template <typename NSLaw, typename RhsT, typename ResT>
  bool tryZero(const NSLaw &law, const RhsT &b, ResT &x, typename GlobalProblemTraits::DynVector &x_best,
