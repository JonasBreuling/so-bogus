...
struct GMRES : public KrylovSolverBase<GMRES, Matrix, Preconditioner, Traits> {
  ...
  GMRES(const Matrix &A, unsigned maxIters, Scalar tol = NumTraits<Scalar>::epsilon(),
        const Preconditioner *P = nullptr, const typename Base::SignalType *callback = nullptr, unsigned restart = 0)
