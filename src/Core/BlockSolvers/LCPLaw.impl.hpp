...
template <typename Scalar>
Scalar LCPLaw<Scalar>::eval(const unsigned, const typename Traits::Vector &x, const typename Traits::Vector &y) const {
  const Scalar fb = x[0] + y[0] - std::sqrt(x[0] * x[0] + y[0] * y[0]);
  return fb * fb;
}

template <typename Scalar>
bool LCPLaw<Scalar>::solveLocal(const unsigned, const typename Traits::Matrix &A, const typename Traits::Vector &b,
