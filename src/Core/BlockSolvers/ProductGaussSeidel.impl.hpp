...
void DMtStorage<MType, DType, Precompute>::multiply(const Rhs& rhs, Intermediate& itm, Res& res) const {
...
#pragma omp parallel for
  ...
  for (Index i = 0; i < n; ++i) {
    ...
    Base::iterableMatrix().eachBlockOfRow(i, block_solvers_impl::accumulate(m_diagonal.asArray(), m_localMatrices[i]));
  }
...
void ProductGaussSeidel<BlockMatrixType, DiagonalType, PrecomputeDMt>::innerLoop(bool parallelize, const NSLaw& law,
                                                                                 const VecT& b,
...
ProductGaussSeidel<BlockMatrixType, DiagonalType, PrecomputeDMt>::solve(const NSLaw& law, const RhsT& b, ResT& x,
...
typename ProductGaussSeidel<BlockMatrixType, DiagonalType, PrecomputeDMt>::Scalar
ProductGaussSeidel<BlockMatrixType, DiagonalType, PrecomputeDMt>::solveWithLinearConstraints(
