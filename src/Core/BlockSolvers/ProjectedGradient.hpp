...
class ProjectedGradient : public ConstrainedSolverBase<ProjectedGradient<BlockMatrixType>, BlockMatrixType> {
 public:
  ...
  //! Should be in ]0,1[
  void setLineSearchArmijoCoefficient(const Scalar lsArmijoCoefficient) { m_lsArmijoCoefficient = lsArmijoCoefficient; }

  //! Sets the variant that will be used when calling solve() without template arguments
  void setDefaultVariant(projected_gradient::Variant variant) { m_defaultVariant = variant; }
