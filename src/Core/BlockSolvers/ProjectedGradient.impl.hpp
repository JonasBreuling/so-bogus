...
bool test_residual(const ProjectedGradient<BlockMatrixType> &pg, const NSLaw &law, const unsigned pgIter, const VecX &x,
...
template <typename BlockMatrixType, typename NSLaw, typename MatrixT, typename VecX, typename VecB, typename VecBuf,
          typename Scalar>
void armijo_ls(const ProjectedGradient<BlockMatrixType> &pg, const NSLaw &law, const MatrixT &M, const VecX &x,
               const VecB &b, const VecBuf &dir, VecBuf &grad, const Scalar J, Scalar &alpha, VecBuf &xs, Scalar &Js,
               VecBuf &Mx) {
  Js = J;
  for (unsigned lsIter = 0; lsIter < pg.lineSearchIterations(); ++lsIter, alpha *= pg.lineSearchPessimisticFactor()) {
    xs = x + alpha * dir;
...
typename ProjectedGradient<BlockMatrixType>::Scalar ProjectedGradient<BlockMatrixType>::solve(const NSLaw &law,
                                                                                              const RhsT &b,
                                                                                              ResT &x) const {
