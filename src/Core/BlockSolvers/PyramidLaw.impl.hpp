...
PyramidLaw<Dimension, Scalar, DeSaxceCOV>::PyramidLaw(const unsigned n, const double *mu) : m_mu(mu), m_n(n) {}
...
void PyramidLaw<Dimension, Scalar, DeSaxceCOV>::projectOnConstraint(const unsigned problemIndex,
  ...
  if (diff.maxCoeff(&kMax) > 0) {
    TgComp signs;
    for (unsigned k = 0; k < Dimension - 1; ++k) signs[k] = x[1 + k] > 0 ? 1 : -1;
    ...
    if (mu * Traits::np(xproj) >= Traits::tp(xproj).template lpNorm<Eigen::Infinity>()) {
      x = xproj;
      ok = true;
    }
...
bool PyramidLaw<Dimension, Scalar, DeSaxceCOV>::solveLocal(const unsigned problemIndex,
                                                           ...
                                                           const Scalar) const {
  static_assert(DeSaxceCOV, "NOT_IMPLEMENTED");
