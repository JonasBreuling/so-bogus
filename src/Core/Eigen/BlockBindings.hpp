...
#ifndef BOGUS_EIGEN_BLOCK_BINDINGS_HPP
#define BOGUS_EIGEN_BLOCK_BINDINGS_HPP
...
static_assert(EIGEN_VERSION_AT_LEAST(5, 0, 0), "bogus requires Eigen 5 or later");

namespace bogus {

// Block utility functions for Eigen dense types (transpose_block,
// is_zero, resize, set_identity)

template <typename EigenDerived>
inline bool is_zero(const Eigen::MatrixBase<EigenDerived>& block, typename EigenDerived::Scalar precision) {
...
template <typename EigenDerived>
inline bool is_zero(const Eigen::SparseMatrixBase<EigenDerived>& block, typename EigenDerived::Scalar precision) {
  return block.isZero(precision);
}
...
struct BlockTraits<Eigen::Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols> > {
  ...
  // Manipulates _Options so that row and column vectors have the correct RowMajor value
  // Should we default to _Options ^ Eigen::RowMajor ?
  using TransposeStorageType = Eigen::Matrix<_Scalar, _Cols, _Rows,
                                             (_Options | ((_Cols == 1 && _Rows != 1) ? Eigen::RowMajor : 0)) &
                                                 ~((_Rows == 1 && _Cols != 1) ? Eigen::RowMajor : 0),
                                             _MaxCols, _MaxRows>;
};

// Block/block product return type
template <typename _Scalar, int _Rows, int _Cols, int _Options, int _MaxRows, int _MaxCols, typename _Scalar2,
          int _Rows2, int _Cols2, int _Options2, int _MaxRows2, int _MaxCols2, bool TransposeLhs, bool TransposeRhs>
struct BlockBlockProductTraits<Eigen::Matrix<_Scalar, _Rows, _Cols, _Options, _MaxRows, _MaxCols>,
                               Eigen::Matrix<_Scalar2, _Rows2, _Cols2, _Options2, _MaxRows2, _MaxCols2>, TransposeLhs,
                               TransposeRhs> {
  using ReturnType = Eigen::Matrix<_Scalar, (TransposeLhs ? _Cols : _Rows), (TransposeRhs ? _Rows2 : _Cols2), _Options,
                                   (TransposeLhs ? _MaxCols : _MaxRows), (TransposeRhs ? _MaxRows2 : _MaxCols2)>;
};

template <typename Derived>
inline typename Eigen::internal::plain_matrix_type<Derived>::type get_mutable_vector(
...
struct EigenBlockWrapper : public Eigen::EigenBase<EigenBlockWrapper<Derived> > {
  using Nested = EigenBlockWrapper;
  using NestedExpression = EigenBlockWrapper;
  using PlainObject = EigenBlockWrapper;
  using Traits = Eigen::internal::traits<EigenBlockWrapper<Derived> >;
  ...
  EigenBlockWrapper(const Derived& obj_, Scalar s = 1) : obj(obj_), scaling(s) {}
  ...
  inline EigenBlockWrapper operator*(const Scalar& scalar) const { return EigenBlockWrapper(obj, scaling * scalar); }

  inline friend EigenBlockWrapper operator*(const Scalar& scalar, const EigenBlockWrapper& matrix) {
    return EigenBlockWrapper(matrix.obj, matrix.scaling * scalar);
  }

  // Product with other Eigen expr
  template <typename EigenDerived>
  inline Eigen::Product<EigenBlockWrapper, EigenDerived> operator*(const EigenDerived& rhs) const {
    return Eigen::Product<EigenBlockWrapper, EigenDerived>(*this, rhs);
  }
  template <typename EigenDerived>
  friend inline Eigen::Product<EigenDerived, EigenBlockWrapper> operator*(const EigenDerived& lhs,
...
struct BlockEigenProduct : public Eigen::Product<Lhs, Rhs> {
  using Base = Eigen::Product<Lhs, Rhs>;

  BlockEigenProduct(const Lhs& lhs, const Rhs& rhs) : Base(lhs, rhs) {}
};

template <typename Derived, typename EigenDerived>
BlockEigenProduct<EigenBlockWrapper<Derived>, EigenDerived> block_eigen_product(const Derived& matrix,
                                                                                const EigenDerived& vector,
                                                                                typename Derived::Scalar scaling = 1) {
  return BlockEigenProduct<EigenBlockWrapper<Derived>, EigenDerived>(EigenBlockWrapper<Derived>(matrix, scaling),
                                                                     vector);
}

template <typename Derived, typename EigenDerived>
BlockEigenProduct<EigenDerived, EigenBlockWrapper<Derived> > eigen_block_product(const EigenDerived& vector,
                                                                                 const Derived& matrix,
                                                                                 typename Derived::Scalar scaling = 1) {
  return BlockEigenProduct<EigenDerived, EigenBlockWrapper<Derived> >(vector,
                                                                      EigenBlockWrapper<Derived>(matrix, scaling));
}

template <typename Derived, typename EigenDerived>
struct block_product_impl<bogus::mv_impl::EigenBlockWrapper<Derived>, EigenDerived> {
  using Lhs = bogus::mv_impl::EigenBlockWrapper<Derived>;
  using Rhs = EigenDerived;
...
struct block_product_impl<EigenDerived, bogus::mv_impl::EigenBlockWrapper<Derived> > {
  using Lhs = EigenDerived;
  using Rhs = bogus::mv_impl::EigenBlockWrapper<Derived>;
  using Scalar = typename Derived::Scalar;
...
struct generic_product_impl<bogus::mv_impl::EigenBlockWrapper<Derived>, EigenDerived, SparseShape, DenseShape,
                            ProductType>
    : public generic_product_impl_base<bogus::mv_impl::EigenBlockWrapper<Derived>, EigenDerived,
                                       generic_product_impl<bogus::mv_impl::EigenBlockWrapper<Derived>, EigenDerived,
  ...
  using product_impl = bogus::mv_impl::block_product_impl<Lhs, RhsNested>;
...
template <typename Derived, typename EigenDerived, int ProductType>
struct generic_product_impl<EigenDerived, bogus::mv_impl::EigenBlockWrapper<Derived>, DenseShape, SparseShape,
                            ProductType>
    : public generic_product_impl_base<EigenDerived, bogus::mv_impl::EigenBlockWrapper<Derived>,
                                       generic_product_impl<EigenDerived, bogus::mv_impl::EigenBlockWrapper<Derived>,
  ...
  using product_impl = bogus::mv_impl::block_product_impl<LhsNested, Rhs>;
...
struct evaluator<CwiseBinaryOp<internal::scalar_product_op<Scalar1, Scalar2>,
  ...
  using XprType = CwiseBinaryOp<internal::scalar_product_op<Scalar1, Scalar2>,
                                const CwiseNullaryOp<internal::scalar_constant_op<Scalar1>, Plain1>,
                                const Product<bogus::mv_impl::EigenBlockWrapper<Derived>, Rhs, DefaultProduct> >;
  using Base = evaluator<Product<bogus::mv_impl::EigenBlockWrapper<Derived>, Rhs, DefaultProduct> >;

  explicit evaluator(const XprType& xpr)
      : Base(bogus::mv_impl::EigenBlockWrapper<Derived>(xpr.rhs().lhs().obj,
...
struct traits<bogus::mv_impl::EigenBlockWrapper<Derived> > {
  ...
  using StorageIndex = typename Derived::Index;
...
mv_impl::BlockEigenProduct<mv_impl::EigenBlockWrapper<Derived>, EigenDerived> operator*(
    const BlockObjectBase<Derived>& lhs, const Eigen::MatrixBase<EigenDerived>& rhs) {
...
template <typename Derived, typename EigenDerived>
mv_impl::BlockEigenProduct<mv_impl::EigenBlockWrapper<Derived>, EigenDerived> operator*(
    const Scaling<Derived>& lhs, const Eigen::MatrixBase<EigenDerived>& rhs) {
  assert(rhs.rows() == lhs.cols());
...
mv_impl::BlockEigenProduct<EigenDerived, mv_impl::EigenBlockWrapper<Derived> > operator*(
    const Eigen::MatrixBase<EigenDerived>& lhs, const BlockObjectBase<Derived>& rhs) {
...
template <typename Derived, typename EigenDerived>
mv_impl::BlockEigenProduct<EigenDerived, mv_impl::EigenBlockWrapper<Derived> > operator*(
    const Eigen::MatrixBase<EigenDerived>& lhs, const Scaling<Derived>& rhs) {
  assert(lhs.cols() == rhs.rows());
...
#endif  // BOGUS_EIGEN_BLOCK_BINDINGS_HPP
