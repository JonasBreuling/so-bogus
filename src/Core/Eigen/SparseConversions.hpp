...
void convert(const Eigen::SparseMatrixBase<EigenDerived>& source, SparseBlockMatrixBase<BogusDerived>& dest,
  ...
  static_assert((((bool)Eigen::SparseMatrixBase<EigenDerived>::IsRowMajor) ^ ((bool)Traits::is_col_major)),
  ...
  for (Index outer = 0; outer < dest.majorIndex().outerSize(); ++outer) {
    ...
    for (Index i = 0; i < blockSize; ++i) {
      for (typename EigenDerived::InnerIterator innerIt(source.derived(), outer * blockSize + i); innerIt; ++innerIt) {
...
void convert(const SparseBlockMatrixBase<BogusDerived>& source,
  ...
  const std::vector<Index>& outerOffsets = ((bool)EigenMatrixType::IsRowMajor) == ((bool)Traits::is_col_major)
  ...
  for (Index outerBlock = 0; outerBlock < index.outerSize(); ++outerBlock) {
    for (Index outer = outerOffsets[outerBlock]; outer < outerOffsets[outerBlock + 1]; ++outer) {
      dest.startVec(outer);
      for (typename IndexGetter::ReturnType::InnerIterator it(index, outerBlock); it; ++it) {
