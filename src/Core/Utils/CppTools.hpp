...
template <typename T>
struct HasConstTransposeReturnType<T, std::void_t<typename T::ConstTransposeReturnType>> : std::true_type {};
...
class ConstMappedArray {
 public:
  ...
  ConstMappedArray(const Element* data, std::size_t size) : m_data(data), m_size(size) {}
