...
struct LinearSolverBase {
  ...
  using UnderlyingBlockTraits = BlockTraits<typename LinearSolverTraits<Derived>::MatrixType>;
  using Scalar = typename UnderlyingBlockTraits::Scalar;
  enum {
...
struct BlockBlockProductTraits<Factorization<LhsBlockT, ImplType>, RhsBlockT, TransposeLhs, TransposeRhs> {
  using LhsMatrixT = typename LinearSolverTraits<Factorization<LhsBlockT, ImplType> >::MatrixType;
  using ReturnType = typename BlockBlockProductTraits<LhsMatrixT, RhsBlockT, TransposeLhs, TransposeRhs>::ReturnType;
};
...
struct BlockBlockProductTraits<LU<LhsBlockT>, RhsBlockT, TransposeLhs, TransposeRhs> {
  using LhsMatrixT = typename LinearSolverTraits<LU<LhsBlockT> >::MatrixType;
  using ReturnType = typename BlockBlockProductTraits<LhsMatrixT, RhsBlockT, TransposeLhs, TransposeRhs>::ReturnType;
};

template <typename LhsBlockT, typename RhsBlockT, bool TransposeLhs, bool TransposeRhs>
struct BlockBlockProductTraits<LDLT<LhsBlockT>, RhsBlockT, TransposeLhs, TransposeRhs> {
  using LhsMatrixT = typename LinearSolverTraits<LDLT<LhsBlockT> >::MatrixType;
  using ReturnType = typename BlockBlockProductTraits<LhsMatrixT, RhsBlockT, TransposeLhs, TransposeRhs>::ReturnType;
};
