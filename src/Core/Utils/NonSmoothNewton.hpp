...
#ifndef BOGUS_NS_NEWTON_HPP
#define BOGUS_NS_NEWTON_HPP
...
class NonSmoothNewton {
 public:
  using Traits = typename NSFunction::Traits;
  using Scalar = typename Traits::Scalar;
  using Vector = typename Traits::Vector;
  using Matrix = typename Traits::Matrix;

  NonSmoothNewton(const NSFunction &func, Scalar tol) : m_func(func), m_tol(tol), m_maxIters(20) {}
