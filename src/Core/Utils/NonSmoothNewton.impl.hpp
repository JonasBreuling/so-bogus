...
#ifndef BOGUS_NS_NEWTON_IMPL_HPP
#define BOGUS_NS_NEWTON_IMPL_HPP
...
typename NonSmoothNewton<NSFunction>::Scalar NonSmoothNewton<NSFunction>::solve(Vector& x) const {
  ...
  for (unsigned iter = 0; iter < m_maxIters; ++iter) {
    ...
    if (proj > 0 || proj * proj < sigma2 * dx.squaredNorm() * dPhi_dx.squaredNorm()) dx *= alpha;
