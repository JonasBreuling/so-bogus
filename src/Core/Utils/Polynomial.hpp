...
struct PossiblyDegenerateRootsFinder {
  static unsigned getRealRoots(Scalar* coeffs, Scalar* realRoots, RealRootsFilter filter = AllRoots);
};
...
*/
template <unsigned Dimension, typename Scalar>
unsigned getRealRoots(const Scalar (&coeffs)[Dimension], Scalar (&realRoots)[Dimension],
...
*/
template <unsigned Dimension, typename Scalar>
unsigned getRealRoots(Scalar (&coeffs)[Dimension + 1], Scalar (&realRoots)[Dimension],
                      RealRootsFilter filter = AllRoots) {
