...
unsigned RootsFinder< Dimension, Scalar>::getRealRoots(
	const Scalar *coeffs, Scalar *realRoots, RealRootsFilter filter )
{
    ...
    const auto& ev =
        Eigen::EigenSolver< typename CM::BaseType >( matrix ).eigenvalues();
	...
	Scalar *coeffs, Scalar *realRoots, RealRootsFilter filter )
{
	...
	{
		return PossiblyDegenerateRootsFinder< Dimension - 1, Scalar >
			::getRealRoots( coeffs, realRoots, filter ) ;
	}
