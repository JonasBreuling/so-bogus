...
template <DenseIndexType Dimension, typename Scalar>
void FBBaseFunction<Dimension, Scalar>::computeJacobian(const Scalar mu, const Vector& x, const Vector& y, Vector& fb,
...
void FBBaseFunction<Dimension, Scalar>::compute(const Vector& x, const Vector& y, Vector& fb, Matrix& dFb_dx,
                                                Matrix& dFb_dy) {
  ...
  if (JacobianAsWell) {
    ...
    if (NumTraits<Scalar>::isZero(rlambda2)) {
      ...
      if (NumTraits<Scalar>::isZero(rlambda1)) {
        ...
        Traits::ttb(invLz) =
            (det * Traits::TgMatrix::Identity(d - 1, d - 1) + Traits::tp(z) * Traits::tp(z).transpose()) / z[0];
        invLz /= det;
...
void FischerBurmeister<Dimension, Scalar, DeSaxceCOV>::compute(const Scalar mu, const Vector& x, const Vector& y,
                                                               Vector& fb) {
  if (DeSaxceCOV) {
    Vector yt(y);
...
template <DenseIndexType Dimension, typename Scalar, bool DeSaxceCOV>
void FischerBurmeister<Dimension, Scalar, DeSaxceCOV>::compute(const Vector& x, Vector& fb) const {
...
template <DenseIndexType Dimension, typename Scalar, bool DeSaxceCOV>
void FischerBurmeister<Dimension, Scalar, DeSaxceCOV>::computeJacobian(const Vector& x, Vector& fb,
                                                                       Matrix& dFb_dx) const {
  const Vector xs = m_scaling * x;
  Vector y(m_A * x + m_b);
  Scalar s = 0.;
  ...
  if (DeSaxceCOV && !NumTraits<Scalar>::isZero(s)) {
    Traits::tc(dFb_dy).noalias() += Traits::nc(dFb_dy) * (m_mu / s) * Traits::tp(y).transpose();
  }
