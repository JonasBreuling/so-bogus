...
struct AnalyticLocalSOCSolver<3u, Scalar, true> {
  ...
  static bool solveOrthogonality(const typename Traits::Matrix &W, const typename Traits::Vector &b,
    ...
    const Vec2 B(Wbar(1, 1) * bbar[0] - Wbar(1, 0) * bbar[1], Wbar(0, 0) * bbar[1] - Wbar(0, 1) * bbar[0]);
    const Scalar C = Wbar.determinant() - wT.dot(B);
    const Scalar D = wN * wN / (mu * mu);

    const Scalar coeffs[4] = {C * C - D * B.squaredNorm(), 2 * (C * A - D * bbar.dot(B)),
                              2 * C + A * A - D * bbar.squaredNorm(), 2 * A};

    Scalar roots[4];
    const unsigned nRoots = polynomial::getRealRoots(coeffs, roots, polynomial::StrictlyPositiveRoots);
...
struct AnalyticLocalSOCSolver<3u, Scalar, false> {
  ...
  static bool solveOrthogonality(const typename Traits::Matrix &W, const typename Traits::Vector &b,
    ...
    Scalar roots[4];
    const unsigned nRoots = bogus::polynomial::getRealRoots(coeffs, roots, bogus::polynomial::AllRoots);
...
Scalar LocalSOCSolver<Dimension, Scalar, DeSaxceCOV, Strat>::solve(const typename Traits::Matrix &A,
                                                                   const typename Traits::Vector &b,
                                                                   typename Traits::Vector &x, const Scalar mu,
                                                                   const Scalar tol, const Scalar scaling) {
  if (mu < 0) {
