...
void applyPermutation(const std::vector<std::size_t> &permutation, Eigen::MatrixBase<EigenDerived> &vec,
                      const Index *offsets) {
...
static double solve(const DualFrictionProblem<Dimension> &dual, const ConstrainedSolverBase<Method, MatrixT> &gs,
                    double *r, const bool staticProblem) {
  typename Eigen::VectorXd::MapType r_map(r, dual.W.rows());

  if (dual.permuted()) applyPermutation<Dimension>(dual.permutation(), r_map, dual.W.majorIndex().innerOffsetsData());

  double res =
      staticProblem
          ? gs.solve(typename DualFrictionProblem<Dimension>::SOCLawType(dual.W.rowsOfBlocks(), dual.mu.data()), dual.b,
                     r_map)
          : gs.solve(typename DualFrictionProblem<Dimension>::CoulombLawType(dual.W.rowsOfBlocks(), dual.mu.data()),
                     dual.b, r_map);
...
template <unsigned Dimension, typename Method, typename MatrixT>
static double eval(const DualFrictionProblem<Dimension> &dual, const ConstrainedSolverBase<Method, MatrixT> &gs,
                   const double *r_data, const bool staticProblem) {
  Eigen::VectorXd r = Eigen::VectorXd::Map(r_data, dual.W.rows());

  if (dual.permuted()) applyPermutation<Dimension>(dual.permutation(), r, dual.W.majorIndex().innerOffsetsData());

  const Eigen::VectorXd u = dual.W * r + dual.b;

  double res =
      staticProblem
          ? gs.eval(typename DualFrictionProblem<Dimension>::SOCLawType(dual.W.rowsOfBlocks(), dual.mu.data()), u, r)
          : gs.eval(typename DualFrictionProblem<Dimension>::CoulombLawType(dual.W.rowsOfBlocks(), dual.mu.data()), u,
                    r);
...
static double solveCadoux(const DualFrictionProblem<Dimension> &problem,
                          ConstrainedSolverBase<Method, MatrixT> &minimizer, double *r, const unsigned cadouxIterations,
  ...
  const double res =
      solveCadoux<Dimension>(problem.W, problem.b.data(), problem.mu.data(), minimizer, r, cadouxIterations, callback);
...
static double solveCadouxVel(const DualFrictionProblem<Dimension> &problem,
                             ConstrainedSolverBase<Method, MatrixT> &minimizer, double *u,
                             const unsigned cadouxIterations, const Signal<unsigned, double> *callback) {
  ...
  const double res = solveCadouxVel<Dimension>(problem.W, problem.b.data(), problem.mu.data(), minimizer, u,
                                               cadouxIterations, callback);
