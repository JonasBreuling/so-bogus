...
void MecheFrictionProblem::ackCurrentResidual(unsigned GSIter, double err) {
  if (m_out) {
    *m_out << "Finished iteration " << GSIter << " with residual " << err << std::endl;
  }
...
void MecheFrictionProblem::fromPrimal(
    ...
    const double
        *E_in,  //!< array of size \f$ n \times d \times d \f$ giving the \a n normals followed by the \a n tangent
                //!< vectors (and by again \a n tangent vectors if \a d is 3). Said otherwise, \a E is a \f$ (nd) \times
                //!< d \f$ matrix, stored column-major, formed by \a n blocks of size \f$ d \times d \f$ with each block
                //!< being an orthogonal matrix (the transition matrix from the world space coordinates \f$ (x_1, x_2,
                //!< x_3) \f$ to the local coordinates \f$ (x_N, x_{T1}, x_{T2}) \f$
    const double *w_in,        //!< array of size \a nd, the constant term in \f$ u = H v + w \f$
    const int *const ObjA,     //!< array of size \a n, the first object involved in the \a i-th contact (must be an
                               //!< internal object) (counted from 0)
    const int *const ObjB,     //!< array of size \a n, the second object involved in the \a i-th contact (-1 for an
                               //!< external object) (counted from 0)
    const double *const HA[],  //!< array of size \a n, containing pointers to a dense, colum-major matrix of size <c>
...
#pragma omp parallel for
  ...
  for (std::ptrdiff_t i = 0; i < (std::ptrdiff_t)n_in; ++i) {
    ...
    if (ObjB[i] == -1) {
      m_primal->H.insert(i, ObjA[i]) = Et * Eigen::MatrixXd::Map(HA[i], 3, ndof[ObjA[i]]);
    } else if (ObjB[i] == ObjA[i]) {
      m_primal->H.insert(i, ObjA[i]) =
          Et * (Eigen::MatrixXd::Map(HA[i], 3, ndof[ObjA[i]]) - Eigen::MatrixXd::Map(HB[i], 3, ndof[ObjA[i]]));
    } else {
      m_primal->H.insert(i, ObjA[i]) = Et * Eigen::MatrixXd::Map(HA[i], 3, ndof[ObjA[i]]);
      m_primal->H.insert(i, ObjB[i]) = -Et * Eigen::MatrixXd::Map(HB[i], 3, ndof[ObjB[i]]);
    }
  }
  m_primal->H.finalize();
...
unsigned MecheFrictionProblem::nDegreesOfFreedom() const { return m_primal ? m_primal->M.rows() : 0u; }
...
double MecheFrictionProblem::solve(double *r, double *v, int maxThreads, double tol, unsigned maxIters,
                                   bool staticProblem, double regularization, bool useInfinityNorm,
                                   bool useProjectedGradient, unsigned cadouxIters) {
  Options options;
...
double MecheFrictionProblem::solve(double *r, double *v, const Options &options, bool staticProblem,
  ...
  if (options.algorithm == ADMM || options.algorithm == DualAMA) {
    ...
    if (!v) {
      v_data =
          m_primal->MInv * (m_primal->H.transpose() * r_loc - Eigen::VectorXd::Map(m_primal->f, m_primal->H.cols()));
      v = v_data.data();
    }
