...
class MecheFrictionProblem {
 public:
  ...
  struct Options {
    ...
    unsigned cadouxIters;  //!< If staticProblem is false and cadouxIters is greater than zero, use the Cadoux algorithm
                           //!< to solve the friction problem.
  ...
  */
  void fromPrimal(
      ...
      const double
          *E_in,  //!< array of size \f$ n \times d \times d \f$ giving the \a n normals followed by the \a n tangent
                  //!< vectors (and by again \a n tangent vectors if \a d is 3). Said otherwise, \a E is a \f$ (nd)
                  //!< \times d \f$ matrix, stored column-major, formed by \a n blocks of size \f$ d \times d \f$ with
                  //!< each block being an orthogonal matrix (the transition matrix from the world space coordinates \f$
                  //!< (x_1, x_2, x_3) \f$ to the local coordinates \f$ (x_N, x_{T1}, x_{T2}) \f$
  ...
  double solve(
      ...
      double regularization = 0.,  //!< Coefficient to add to the diagonal of static problems / GS regularization
                                   //!< coefficient for friction problems
      bool useInfinityNorm =
          false,  //!< Whether to use the infinity norm to evaluate the residual of the friction problem,
      bool useProjectedGradient = false,  //!< 0 = GS, 1 = PG, 2 = ADMM, 3 = DualAMA.
      unsigned cadouxIters = 0  //!< If staticProblem is false and cadouxIters is greater than zero, use the Cadoux
                                //!< algorithm to solve the friction problem.
  );
