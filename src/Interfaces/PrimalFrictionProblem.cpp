...
template <unsigned Dimension>
double PrimalFrictionProblem<Dimension>::solveWith(ProductGaussSeidelType &pgs, double *r,
                                                   const bool staticProblem) const {
  // b = E' w - H M^-1 f
  const Eigen::VectorXd b =
      E.transpose() * Eigen::VectorXd::Map(w, H.rows()) - H * (MInv * Eigen::VectorXd::Map(f, H.cols()));
...
template <unsigned Dimension>
double PrimalFrictionProblem<Dimension>::solveWith(DualAMAType &dama, double *v, double *r,
                                                   const bool staticProblem) const {
  const Eigen::VectorXd f = Eigen::VectorXd::Map(this->f, M.rows());
  const Eigen::VectorXd w = E.transpose() * Eigen::VectorXd::Map(this->w, H.rows());
