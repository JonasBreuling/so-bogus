...
TEST(SparseBlock, MMult) {
...

  {
    ...
    bogus::SparseBlockMatrix<Eigen::Matrix4d, bogus::flags::UNCOMPRESSED | bogus::flags::SYMMETRIC> mm_t =
        sbm.transpose() * sbm;
    ...
    bogus::SparseBlockMatrix<Eigen::Matrix<double, 4, 3> > mm_t_mt2 =
        2 * mm_t_mt - sbm.transpose() * sbm * sbm.transpose();
  ...
  }
  {
    ...
    bogus::SparseBlockMatrix<Eigen::Matrix4d,
                             bogus::flags::COL_MAJOR | bogus::flags::UNCOMPRESSED | bogus::flags::SYMMETRIC>
        mm_t = sbm.transpose() * sbm;
...
TEST(SparseBlock, Add) {
...

  {
    ...
    bogus::SparseBlockMatrix<Eigen::MatrixXd, bogus::flags::COL_MAJOR> sbm_bis = ssbm + sbm.transpose();
    EXPECT_EQ(sym_res, sbm_bis * rhs);

    bogus::SparseBlockMatrix<Eigen::MatrixXd> sbm_ter = (sbm_bis.transpose() - ssbm).transpose();
